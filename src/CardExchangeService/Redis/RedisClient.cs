...
namespace CardExchangeService.Redis
{
  public class RedisClient : IRedisClient
  {
    ...
    private async void OnExpireDeviceId(RedisChannel channel, RedisValue value)
    {
      ...
      if (channelStr.StartsWith(chanelPrefix) && (string)value == "expire")
      {
        string key = channelStr.Substring(chanelPrefix.Length);
        if (!key.StartsWith(GetGeoEntryKey()))
        {
          await GeoRemove(channelStr.Substring(chanelPrefix.Length));
        }
      }
    }
    ...
    public async Task<bool> SetString(string key, string value)
    {
      ...
      return await db.KeyExpireAsync(key, TimeSpan.FromSeconds(_redisKeyExpireTimeout));
    }
    ...
    public Task<GeoRadiusResult[]> GeoRadiusByMember(string member)
    {
      var db = Redis.GetDatabase();
      return db.GeoRadiusAsync(GetGeoEntryKey(), member, _redisGeoRadius);
    }
